...
class D3Grimme:
    ...
    def __init__(self, elements, coordinates, order=6):
        ...
        polarizabilities = calc.get_property(
            "polarizibilities", atoms=atoms) * (ANGSTROM / BOHR)**3
        c6_coefficients_all = calc.get_property(
            'c6_coefficients',
            atoms=atoms) * EV / HARTREE * (ANGSTROM / BOHR)**6
...
class D4Grimme:
    ...
    def __init__(self, elements, coordinates, order=8, charge=0):
        ...
        charges[0] = charge
        atoms = atoms = ase.Atoms(numbers=elements,
